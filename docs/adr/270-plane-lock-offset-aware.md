...
- **다시 바닥(z=0)에 그리기**: **Home 키** · **F5** · **🏠 버튼** · **h 키**
  (카메라 홈 4종) · **우클릭 → 기본 평면으로** · **Top 뷰 전환** → 빈 공간은
  바닥 z=0, 면 위는 여전히 그 면.
...
### F.7 Amendment 3 — 카메라 홈(F5 / 🏠 / h)도 평면 초기화 (2026-07-03)
사용자 요청 — "홈"은 카메라뿐 아니라 드로잉 평면도 기본(z=0)으로 복귀해야 직관적.
카메라 홈 3 트리거(**F5** / **🏠 버튼** / **h 키**)가 `viewport.resetCamera()` 에
더해 `toolManager.resetDrawingPlane()` 도 호출 → 면에 그린 뒤 F5/🏠/h 로도 바닥
(z=0)에 다시 그릴 수 있음. 즉 평면 초기화 진입점 = Home / F5 / 🏠 / h / 우클릭
메뉴 / Top 뷰. 브라우저 검증(F5·🏠 pinned true→false) + 회귀 +4 (H/F5/Home-key/
home-button 이 resetDrawingPlane 호출), KeyboardShortcuts 45/45 PASS.
