...
- **다시 바닥(z=0)에 그리기**: **Home 키** · **F5** · **🏠 버튼** (카메라 홈) ·
  **우클릭 → 기본 평면으로** · **Top 뷰 전환** → 빈 공간은 바닥 z=0, 면 위는
  여전히 그 면.
...
### F.7 Amendment 3 — 카메라 홈(F5 / 🏠)도 평면 초기화 (2026-07-03)
사용자 요청 — "홈"은 카메라뿐 아니라 드로잉 평면도 기본(z=0)으로 복귀해야 직관적.
카메라 홈 트리거(**F5** / **🏠 버튼**)가 `viewport.resetCamera()` 에 더해
`toolManager.resetDrawingPlane()` 도 호출 → 면에 그린 뒤 F5/🏠 로도 바닥(z=0)에
다시 그릴 수 있음. 즉 평면 초기화 진입점 = Home / F5 / 🏠 / 우클릭 메뉴 / Top 뷰.

**Amendment 3.1 (2026-07-03) — 'h' 카메라-홈 단축키 제거.** 사용자 요청("기능키가
너무 많다") — 'h' 는 F5 / 🏠 와 중복이라 카메라-홈 바인딩 제거. 부수 효과(정합):
'h' 는 keyMap 에 원래 `'h': 'sphere'` (Sphere 도구) 로 등록돼 있었으나 카메라-홈
핸들러가 먼저 잡아 **shadowed(dead)** 상태였다 → 제거로 **'h' = Sphere 도구**
정상 복원. 카메라 홈은 F5 + 🏠 2개로 정리. 회귀 +3 순감(H 카메라 2 테스트 제거),
KeyboardShortcuts 43/43 PASS.
