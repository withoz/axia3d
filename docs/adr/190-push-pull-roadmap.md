...
### Phase 3 — UX / CAD parity (예정)
- repeat-last (`lastPPDist` 이미 캐시) / modifier(Ctrl=병합 안 함) / smooth-group
  align(현재 비활성) / 곡면 fallback UI 표시 / MIN_COMMIT_DIST config
- 예상 회귀 +25. 위험 낮음 (TS 중심)
