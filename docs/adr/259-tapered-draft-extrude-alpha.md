...
- **Status**: Accepted (β 구현 완료 — flat-profile frustum + draft-on-solid-face; §D Acceptance Log)
- **Date**: 2026-06-26
...
- **L-259-8 draft-on-solid-face = MoveOnly-taper** (§12) — 기존 solid 면(is_move_only)
  taper 는 profile 보존이 아닌 **ring 이동 + 벽 slant** (no 새 face → K-ε sandwich 없음).
  Scene `exec_push_pull_tapered` 로 dispatch (mesh `create_solid` reject 는 직접-mesh
  caller defense 로 보존). WASM `PushPullDone` arm = draft 성공 (straight fallback 아님).
...
## 12. draft-on-solid-face (2026-07-11) — the v1 rejection lifted

**사용자 결재 "ADR-259 마무리 (taper)" (2026-07-11)**. measure-first 로 doc-lag
확정: β (flat-profile frustum) 는 이미 shipped (19 회귀 green, production dist).
**유일한 gap = 기존 solid 면 draft** — v1 이 K-ε sandwich 우려로 거부(§dispatch
`NotYetSupported`). de-risk sim (`adr259_sim_draft_solid_face_moveonly_manifold`)
으로 **MoveOnly-taper 가 sandwich 를 회피**함을 실증 → 활성.

- **기하**: profile 을 보존하지 않고(=create_solid 의 bottom-cap sandwich 근원),
  기존 solid 면의 boundary ring 을 offset(inward/outward) + 노멀 방향 dist 만큼
  **이동**. 기존 벽은 평면 사다리꼴로 slant (ADR-259 §2 exact-Plane 증명 — MoveOnly
  에도 동일). 새 face 0 → manifold 보존.
- **layer**: mesh `Mesh::push_pull_tapered` (→ `push_pull_move_only_tapered`:
  offset_polygon_2d fail-closed + ring 이동 + 이동 face/벽 Plane surface 재-synthesize)
  → Scene `exec_push_pull_tapered` (exec_push_pull ownership/transaction 1:1 mirror)
  → Scene `exec_create_solid` 의 `ExtrudeTapered` + is_move_only dispatch → WASM
  `create_solid_extrude_tapered` 의 **PushPullDone success arm** → 기존 bridge/
  PushPullTool VCB `거리,각도` 경로 **무변경** (자동 draft).
- **fail-closed (D5) 보존**: steep draft → offset collapse → bail → Scene transaction
  rollback → byte-identical (straight fallback 0). WASM Error arm → false → Toast.

### D. Acceptance Log

- **flat-profile frustum β (prior, doc-lag)** — `ExtrudeTapered` + `extrude_planar_
  box_tapered` + WASM `create_solid_extrude_tapered` + bridge `createSolidExtrude
  Tapered` + PushPullTool commitTaper(VCB `거리,각도`) + geom2 `offset_polygon_2d`.
  19 회귀 green (geom2 10 + create_solid 9, `adr259_sim_taper_side_faces_exact_
  planes` 포함). production dist 반영. (catalog Status "Proposed/코드0" 는 stale
  doc-lag 였음 — ADR-260/261 이 "ADR-259 답습" 으로 D5/dispatch 인프라 인용.)
- **draft-on-solid-face (2026-07-11)** — de-risk sim → mesh `push_pull_move_only_
  tapered`/`push_pull_tapered` + Scene `exec_push_pull_tapered` + `exec_create_solid`
  ExtrudeTapered/is_move_only dispatch + WASM PushPullDone arm + step6 guard 재작성.
  회귀 +5: axia-geo `adr259_sim_draft_solid_face_moveonly_manifold`; axia-core
  `adr259_scene_draft_solid_box_top_moveonly` + `adr259_scene_draft_solid_steep_
  reject_rolls_back`; step6 `adr259_tapered_wasm_success_arms` (rewrite); E2E
  `adr-259-draft-on-solid-face.spec.ts` ×2 (real Chromium: box top draft → 6-face
  manifold / steep reject → byte-identical). TS 변경 0 (기존 VCB 경로 자동).
  workspace 3016 passed / 0 failed / 1 ignored. E2E 2/2.

### E. Lessons

- **L1 measure-first 가 doc-lag 노출** — catalog "Proposed/코드0" vs 실제 shipped
  β. 판정 전 코드/테스트 대조 (메타-원칙 #4 SSOT, [[project-engine-state-and-doc-lag]]).
- **L2 v1 "future op" 거부는 보수적이었다** — de-risk sim 이 MoveOnly-taper 의
  manifold 안전성을 실증 → 거부 lift. "면깨짐 우려" 는 profile-preservation
  경로에만 해당 (MoveOnly 는 무관).
- **L3 dispatch 로 활성, mesh reject 는 defense 보존** — Scene 가 is_move_only 를
  intercept; mesh `create_solid` reject 는 직접-mesh caller (MCP/스크립트) defense
  로 유지. 두 layer 의 contract 가 상충 아닌 상보.
- **L4 TS 변경 0** — 기존 taper VCB 경로가 `Command::CreateSolid{ExtrudeTapered}`
  로 라우팅 → Scene dispatch 확장만으로 tool 자동 지원 (ADR-046 P31 #4 additive).
