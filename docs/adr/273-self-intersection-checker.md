...
## 4. 성능 벤치 (2026-07-03, node-WASM 실측)

분리된 박스 grid 로 `detectSelfIntersections` scaling 측정 (median of 3):

| faces | ms | ms/face | ms/face²(×1e6) |
|-------|-----|---------|----------------|
| 600   | 1.2 | 0.0020  | 3.41 |
| 2400  | 3.9 | 0.0016  | 0.68 |
| 5400  | 7.8 | 0.0014  | 0.27 |
| 9600  | 19.4| 0.0020  | 0.21 |

**ms/face 거의 상수 + ms/face² 급감 → near-linear (quadratic 아님)**. 공간 grid
(D3)가 목적대로 동작. 9600 face 에서 19.4ms → 게이트 2×/op 이어도 ~40ms (메타-
원칙 #11 Heavy budget 500ms 의 8%). 회귀 `detect_scales_to_many_disjoint_faces`
(900 quad, quadratic 회귀 가드).
...
- 한 cell 에 수천 face 밀집(pathological clustering) 시 grid 열화 — 실사용
  mesh 는 공간 분산이라 무해, 필요 시 계층 grid/BVH.
...
- `detect_scales_to_many_disjoint_faces` (grid quadratic 회귀 가드)
