...
- **F-L4 (cross-drill 지원은 future).** 진짜 T-교차 구멍은 Boolean(빼기)
  급 위상 → 별도 ADR. 현재는 **위치 이동** 안내.
  - **정정 (2026-07-03):** 초기 메시지는 "Boolean(빼기)을 사용하세요" 였으나
    실측(probe)으로 **막다른 길** 확인 — 구멍 있는 솔리드는 캡이 ring-with-hole
    (multi-loop)이라 `boolean_op` 이 `multi-loop boolean not yet supported` 로
    거부. cross-drill 은 항상 기존 구멍이 있으므로 Boolean 우회 불가. 메시지를
    "구멍 위치를 옮겨 주세요" 로 정정. multi-loop Boolean 은 별도 트랙(ADR-255
    C1 defer 였으나 본 시연으로 실수요 확인).
