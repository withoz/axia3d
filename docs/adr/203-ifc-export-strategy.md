...
## 26. I-3-swept Acceptance — IfcExtrudedAreaSolid (상용 BIM 의 지배적 형태)

지금까지 임포트는 **B-rep** (`IfcAdvancedBrep` / `IfcFacetedBrep`) 만 읽었다.
우리 파일과 AixxiA 파일은 그 형태다. 하지만 **Revit·ArchiCAD 는 거의 모든 벽·
슬래브·기둥을 `IfcExtrudedAreaSolid`** — 2D 프로파일을 방향으로 depth 만큼 압출
— 로 쓴다. I-2 는 이걸 "미지원 형상" 으로 정직히 보고만 했다. 이제 임포트한다.

- **`extruded_area_solid_loops`**: `IfcExtrudedAreaSolid(SweptArea, Position,
  ExtrudedDirection, Depth)` 를 읽어 프로파일을 Position 프레임으로 배치하고,
  ExtrudedDirection 방향으로 depth 만큼 밀어, **두 cap + 옆면 quad** 를 일반
  face loops 로 생성 → 기존 폴리곤 경로가 나머지 (공간 그룹·재-export) 를 그대로
  처리. winding 은 엔진이 outward 로 정규화 (ADR-007) 하므로 일관 입력이면
  닫힌 solid.
- **프로파일 3종** (`parse_profile`): `IfcRectangleProfileDef` (벽·각기둥),
  `IfcCircleProfileDef` (원기둥, chord-tol tessellate), `IfcArbitraryClosed
  ProfileDef` + `IfcPolyline` (임의 다각형 슬래브). 2D 배치
  (`IfcAxis2Placement2D`) 지원.
- **정직한 미지원**: 프로파일을 못 읽거나 (composite curve, I-beam, voids)
  depth 0 이면 face 0 + 경고 → 조작된 형상 안 만듦.
- **classifier**: `SUPPORTED_GEOMETRY` 에 `IFCEXTRUDEDAREASOLID` 추가.
  기존 "미지원 보고" 테스트는 `IfcSweptDiskSolid` (여전히 미지원) 로 교체.

### 실측 (닫힌 solid 검증)

| 프로파일 | 결과 |
|---|---|
| Rectangle 4000×200 → 3000 (벽) | 6면/8정점, **closedSolid**, bbox 정확 |
| Circle r300 → 4000 (원기둥) | 275면, **closedSolid**, bbox 600×600×4000 |
| L-폴리라인 → 500 (슬래브) | 8면/12정점, **closedSolid** |

실제 앱: 압출 벽 + 원기둥이 정확한 3D solid 로 렌더 (I-4 배치로 기둥 3m 옆),
콘솔 오류 0. **B-rep 경로 완전 무변경** (실파일 96면·3면 그대로).

### 회귀

- axia-ifc **+3**: `an_extruded_area_solid_is_supported_geometry` (classifier)
  / `an_extruded_rectangle_becomes_a_prism` (6면 + bbox) / `an_extruded_
  polyline_becomes_a_prism_and_a_circle_is_round` (삼각형 5면, 원 다면). 기존
  미지원-보고 테스트는 `IfcSweptDiskSolid` 로 갱신 (미지원 커버 유지).
  mutation 확인: 압출 디스패치를 끄면 실패.
- 워크스페이스 **3161 passed / 0 failed / 1 ignored**(doc fence), vitest **2940**,
  tsc·build·ADR 카탈로그 통과.

### 사용자 가치 — 상용 BIM 임포트 unlock

이전엔 Revit/ArchiCAD 파일을 열면 벽·슬래브가 "미지원" 으로 안 들어왔다. 이제
그들의 **지배적 형상** (압출 솔리드) 이 실제 3D 로 들어온다. "상용 BIM 미검증"
갭의 핵심 (형상) 이 좁혀졌다 — 남은 건 사용자의 실제 Revit/ArchiCAD 파일 열기.

### 남은 한계

- **프로파일 voids** (중공 단면) — `IfcArbitraryProfileDefWithVoids` 는 이제
  **압출에서 지원** (§26-amendment). 회전(`IfcRevolvedAreaSolid`) 프로파일의
  void 는 아직 outer 만.
- Composite/indexed 곡선 프로파일 (`IfcCompositeCurve` / `IfcIndexedPolyCurve`),
  I-beam 등 파라메트릭 단면 (`IfcIShapeProfileDef` 등) 미지원.
- 다른 swept solid: `IfcRevolvedAreaSolid` (회전), `IfcSweptDiskSolid` (파이프),
  `IfcBooleanResult` (CSG — 개구부 있는 벽) 미지원 → **§27 에서 solid−solid CSG
  임포트.**

### §26-amendment — 프로파일 voids (중공 단면 → 튜브)

`IfcArbitraryProfileDefWithVoids` (outer 경계 + N 개의 inner 곡선 = 구멍) 를
압출하면 **관통 구멍이 있는 solid** (튜브 = genus-1) 가 된다. 속 빈 swept-disk
튜브(§33)가 이미 쓰는 패턴 — outer wall + inner wall + annular cap — 을 그대로
다각형 프로파일에 적용.

- **`parse_profile_voids`** (`ifc_geometry.rs`): `IfcArbitraryProfileDefWithVoids`
  의 arg 3 (`InnerCurves`) 를 각각 `polyline_2d` 로 읽어 2D 루프 리스트 반환. 다른
  프로파일은 빈 리스트 (voids 없음). `parse_profile` 은 outer 만 (기존 caller 무영향).
- **`extruded_area_solid_loops`**: 각 void 를 base/top 프레임에 매핑 → (1) 두 캡이
  `inners` 로 구멍을 물고 (`add_face_with_holes`), (2) void 마다 자체 측벽 링(outer
  wall 과 **반대 winding** — 노멀이 구멍 안쪽; 커널이 outward 정규화). void 의
  bottom/top 엣지가 캡의 hole loop 와 일치 → watertight.
- **결과**: face = 캡 2 + outer 측벽 N + void 측벽 M. ADR-007 winding 정규화 +
  spatial-hash dedup 이 shared vertex 를 용접.

실측: 4×2 m 사각형 + 2×1 m void, 3 m 압출 → **10 face** (캡 2 + outer 4 + void 4),
`verify_face_invariants().is_valid()` = **valid watertight** (genus-1), AABB outer
4000×2000×3000 mm 정확, V=16. node round-trip (컴파일 nodejs 아티팩트) valid=true·0
violation, 라이브 앱 import 렌더 갱신. 회귀 **+2** (`ifc_geometry`
`an_extruded_profile_with_a_void_becomes_a_tube` 10 face·캡 2·hole 4 / WASM
`an_extruded_profile_with_a_void_imports_as_a_valid_tube` valid + AABB). mutation:
`parse_profile_voids` 를 empty 로 하면 10→6 face 로 둘 다 실패 (구멍 안 뚫림).
...
`IfcBooleanClippingResult(.DIFFERENCE., 벽 솔리드, 개구부 솔리드)` — 즉 CSG
트리로 쓴다 (구멍을 그냥 다각형으로 굽지 않는다). §26 이 압출 솔리드를 열었지만
그 위의 boolean 이 없으면 벽은 여전히 "미지원". 본 절이 그 마지막 형상 갭을 닫는다.

- **`ifc_geometry.rs` CSG 트리**: `BoolOp {Union, Subtract, Intersect}` +
  `CsgOperand {Solid(Vec<FaceLoops>), Node(Box<CsgNode>)}` + `CsgNode
  {op, first, second}`. `ElementGeometry.booleans: Vec<CsgNode>` (additive
  필드 — 기존 face 경로 무변경).
- **`parse_boolean`**: `IFCBOOLEANRESULT`/`IFCBOOLEANCLIPPINGRESULT` →
  `.UNION./.DIFFERENCE./.INTERSECTION.` → `BoolOp`. 피연산자는 재귀
  (`parse_boolean_operand`): 중첩 boolean → `Node`, 아니면
  `geometry_face_loops_counted` (§26 디스패처) 로 솔리드. **loop < 4 → None**
  (half-space·퇴화는 솔리드가 아님 → 거부).
- **엔진 boolean 을 임포트 시점에 실행**: `lib.rs` `eval_csg` 가 두 피연산자
  솔리드를 mesh 에 만들고 (`add_face_with_holes` + plane 부착, ADR-087 K-ε)
  `mesh.boolean_solid(a, b, op)` — 우리 자신의 커널로 합친다. `axia_ifc::BoolOp`
  → `axia_geo::operations::boolean::BoolOp`. 결과 face 가 부재의 face.
- **정직한 거부**: 피연산자가 half-space (`IfcHalfSpaceSolid`) 거나 못 만드는
  솔리드면 `None` → 부재를 경고하고 **버린다** (개구부 없는 벽 = 조용히 틀린
  형상 을 만들지 않는다). boolean 이 validity gate (ADR-276) 에 걸려도 마찬가지.
- **배치**: 부재의 `IfcLocalPlacement` 는 boolean 트리 전체에 적용
  (`CsgNode::transform`). 각 피연산자의 자체 `Position` (개구부를 벽 안에
  놓는 것) 은 `extruded_area_solid_loops` 가 이미 적용.
- **classifier**: `SUPPORTED_GEOMETRY` 에 `IFCBOOLEANRESULT` +
  `IFCBOOLEANCLIPPINGRESULT` 추가.

### 실측 (닫힌 solid + 구멍)

| 입력 | 결과 |
|---|---|
| 벽 4×0.2×3 m (압출) | 6면/8정점, **valid closedSolid** |
| 벽 − 창 (boolean, 관통) | **10면/24정점, valid closedSolid**, geometricCracks 0, openBoundaryEdges 0 |
| 벽 − half-space (피연산자 미지원) | 임포트 0 + 경고 (틀린 형상 안 만듦) |

구멍이 뚫린 벽은 앞·뒷면 + 4 터널 벽 = 10면 의 watertight genus-1 solid.

### 외부 커널 교차검증 (web-ifc)

동일한 wall-with-window fixture 를 (project + context 로 완전 배선) **web-ifc**
(IFC.js 뒤의 C++ 커널) 로 열어 메시화: **1 wall, 576 vertex floats, 96 indices**
— 외부 커널도 boolean-clipped 벽을 구멍 뚫린 solid 로 인정. 우리 fixture 가
관대한 자체 파서만 통과하는 게 아니라 well-formed IFC 임을 증명 (web-ifc 가 처음
잡아준 실제 스키마 버그 — 2D 프로파일 Position 에 `IfcAxis2Placement3D` 를 준 것
— 을 고친 뒤).

### 회귀

- axia-ifc **+2**: `a_boolean_clipping_result_parses_as_a_subtract_of_two_solids`
  (Subtract + 두 6면 프리즘 피연산자) / `a_half_space_operand_makes_the_boolean_
  unreadable` (경고, 조용히 안 버림).
- axia-wasm **+2** (WASM 경로, CI 가시): `boolean_clipping_result_imports_as_a_
  holed_solid` (6면 벽 vs >6면 구멍벽 + `verify_face_invariants` valid) /
  `boolean_with_a_half_space_operand_imports_nothing`. mutation 확인:
  `build_csg_operand` 를 None 으로 강제하면 구멍벽 → 0면 → 실패.
- 워크스페이스 **3165 passed / 0 failed / 1 ignored**(doc fence), vitest **2940**,
  tsc·build·ADR 카탈로그·배선 가드 통과. 양 WASM 타깃 byte-parity (4061782).

### 사용자 가치 — 개구부 있는 벽 unlock

이전엔 창·문 뚫린 벽 (상용 BIM 의 표준 표현) 이 안 들어왔다. 이제 그 boolean 이
우리 자신의 커널로 임포트 시점에 풀려 실제 구멍 뚫린 3D solid 가 된다. §26 (압출)
과 함께 상용 BIM 벽 interop 의 마지막 형상 갭을 닫는다.

### 남은 한계

- **half-space 피연산자** (`IfcHalfSpaceSolid` / `IfcPolygonalBoundedHalfSpace`)
  → **§28 에서 지원.**
- **곡면 프로파일 구멍**: 개구부가 원형이면 압출 원기둥 (다각형화) 으로 임포트 —
  boolean 은 되나 곡률 metadata 는 없음.
- 문·창은 여전히 **독립 부재** — boolean 개구부와 `IfcRelVoidsElement` /
  `IfcRelFillsElement` 로 연결 안 됨 (별도 트랙).

## 28. I-3-halfspace Acceptance — IfcHalfSpaceSolid 피연산자 (clipping)

**평면·프리즘으로 잘린 벽이 들어온다.** BIM 은 경사 절단·채널·경계 클립을 boolean
의 피연산자 *half-space* 로 쓴다 — 닫힌 solid 가 아니라 평면의 한쪽 반공간. §27 이
solid−solid 를 열었지만 half-space 피연산자는 거부됐다. 본 절이 두 종류를 지원한다.

- **파싱** (`ifc_geometry.rs`): `CsgOperand::HalfSpace(HalfSpace)` 추가.
  `HalfSpace { base_origin, base_normal, agreement, boundary }`. `parse_half_space`
  는 `IfcHalfSpaceSolid(BaseSurface, AgreementFlag)` 의 BaseSurface 를 `IfcPlane`
  (Position → origin + 법선) 로 읽고, `IfcPolygonalBoundedHalfSpace` 면 Position +
  PolygonalBoundary 폴리곤을 world 로 변환해 `boundary = Some((polygon, extrude_dir))`.
  **곡면 base surface** (`IfcCylindricalSurface` 등) 는 `None` → 거부 (평면만 클립).
- **평가** (`lib.rs`): half-space 는 *minuend* 가 될 수 없으므로 (무한) `eval_csg`
  의 **두 번째 피연산자** 로만 처리 — 첫 피연산자 solid 를 *clip*:
  - **Unbounded** → `mesh.trim_volume_by_plane(a, base_plane, keep_side)`. `keep_above`
    = 법선-양의 쪽 (slice `Side::Above`). op 이 어느 쪽을 남길지 결정.
  - **Polygonally bounded** → 경계 폴리곤을 operand AABB 범위만큼 `extrude_dir` 로
    쓸어 유한 prism 을 만들고, base plane 으로 half-space 쪽만 trim → 유한 clip
    solid → `mesh.boolean_solid(a, clip, op)` (§27 경로 재사용).
- **의미론 (AgreementFlag)**: FALSE → material 이 법선-양의 쪽, TRUE → 음의 쪽.
  `DIFFERENCE` 는 material 없는 쪽을, `INTERSECTION` 은 material 쪽을 남긴다.
  `UNION` (유한 ∪ 무한) 은 ill-defined → 거부.

### 실측 (semantics + clip)

Unbounded — box Z[0,2000], base plane z=1000 (+Z 법선), 4 조합 모두 valid closed
solid, DIFFERENCE↔INTERSECTION 은 정확히 여집합:

| AgreementFlag · op | 남는 쪽 |
|---|---|
| `.F.` DIFFERENCE | Z[0,1000] (material 아닌 아래쪽) |
| `.T.` DIFFERENCE | Z[1000,2000] (반대쪽) |
| `.F.` INTERSECTION | Z[1000,2000] (material 쪽) |
| `.T.` INTERSECTION | Z[0,1000] |

Polygonally bounded (pbhs 지오메트리) — box 2×2×1 m − slot(x[0.75,1.25], z>0.5)
→ **10면 valid closedSolid** (윗면 가운데 채널), geometricCracks 0, live 앱 렌더로
채널 확인, 콘솔 오류 0.

### 외부 커널 교차검증 (web-ifc)

unbounded fixture (project + context 배선) 를 web-ifc (C++ 커널) 로 메시화 — 절단면
높이만큼 (2m box → 1m slab) 잘려 나옴, 우리 결과 (Z[0,1000]) 와 절단량 일치.
`flatTransformation` 이 커널 로컬 프레임이라 절대 위치는 다르나, *반쪽만 남았다* 는
사실이 clip 발화를 확인. 어느 쪽인지는 우리 엔진의 4-조합 flip 실측 + IFC 스펙
정의로 확정 (`.F.` = +법선 쪽 material).

### 회귀

- axia-ifc **+2 net**: `an_unbounded_half_space_operand_parses_as_a_planar_clip`
  / `a_polygonal_bounded_half_space_operand_parses_with_its_polygon` /
  `a_curved_base_surface_half_space_is_unreadable` (기존 "unreadable" 테스트 대체).
- axia-wasm **+2 net** (WASM 경로, CI 가시): `unbounded_half_space_clips_the_operand_
  by_a_plane` (6면 + active z_max≈1000) / `polygonal_bounded_half_space_cuts_a_channel`
  (>6면 valid) / `half_space_on_a_curved_base_imports_nothing`. mutation 확인:
  `apply_half_space` 를 no-op 으로 만들면 두 clip 테스트 실패.
- 워크스페이스 **3169 passed / 0 failed / 1 ignored**(doc fence), vitest **2940**,
  tsc·build·ADR 카탈로그·배선 가드·외부 IFC(web-ifc) 통과. 양 WASM 타깃 byte-parity
  (4068833).

### 사용자 가치 — 경사 절단·채널 벽 unlock

Revit/ArchiCAD 의 클립 (경사 지붕에 맞춘 벽, 경계로 자른 슬래브, 채널) 이 half-space
로 쓰이는데 이제 들어온다. §26 (압출) + §27 (solid CSG) + §28 (half-space) 로 상용
BIM 벽/솔리드 형상의 주요 CSG 경로가 모두 임포트된다.

### 남은 한계

- **곡면 base surface half-space** (`IfcCylindricalSurface` 등 위의 반공간) 미지원 —
  평면 base 만 클립 (별도 트랙).
...
### §29-amendment — IfcRelFillsElement grouping (문·창을 벽 하위로)

기하는 §29 로 완성됐지만 (구멍 + 창), 창이 씬 트리에서 벽과 *형제* 로 storey 아래
평평하게 놓여 있었다. `IfcRelFillsElement(opening, filler)` 을 `IfcRelVoidsElement
(wall, opening)` 과 합성해 **filler → wall** 을 얻고, 문·창을 그 벽 그룹 *하위* 로
넣는다 — 벽을 선택하면 창이 딸려온다.

- **`collect_fills`** (`ifc_geometry.rs`): opening→wall (voids) ∘ opening→filler
  (fills) → `filler #N → wall #M`. `ElementGeometry.fills_wall: Option<u32>` 추가.
- **grouping (lib.rs, I-5 2-pass)**: 부재 그룹을 먼저 모두 만들어 `element_id →
  group` 기록 → 각 부재를 `fills_wall` 있으면 그 벽 그룹 하위로, 없으면 spatial
  container 하위로 parent. (벽 그룹이 창보다 먼저 있어야 하므로 2-pass.)
- 실측 (RDF reference): 트리가 Project → Site → Building → Storey → **Wall (구멍
  뚫린, 10면) → Window (6면)**. Live 앱 `get_all_groups` 로 window.parent == wall
  확인. mutation: `collect_fills` empty → 창이 storey 하위로 → 테스트 실패.
- 회귀 **+2**: `a_window_filling_an_opening_records_the_wall_it_belongs_to` (ifc,
  fills_wall 합성) + `a_window_is_grouped_under_the_wall_it_fills` (WASM 경로, 창
  그룹 parent == 벽 그룹).
...
## 30. I-3-tessellation Acceptance — IfcTriangulatedFaceSet (SketchUp / Revit 메시)

**테셀레이션 모델이 들어온다.** SketchUp·Revit 은 곡면·복잡 형상을 삼각형 메시
(`IfcTriangulatedFaceSet`) 로 내보낸다 — 깔끔한 analytic face 가 아니라 삼각형
soup + 좌표 리스트. 이전엔 "no convertible geometry" 로 통째로 거부됐다 (AixxiA
SketchUp 코퍼스 전체가 이 형식). 본 절이 각 삼각형을 face 로 임포트한다.

- **`cartesian_point_list_3d`** (`ifc_geometry.rs`): `IfcCartesianPointList3D` 의
  inline `((x,y,z), …)` (point ref 리스트가 아닌 좌표 tuple 직접) → `Vec<DVec3>`.
- **`triangulated_face_set_loops`**: `IfcTriangulatedFaceSet(Coordinates, Normals,
  Closed, CoordIndex, PnIndex)` 의 CoordIndex `((1,2,3), …)` (1-based) → 삼각형마다
  3-vertex `FaceLoops`. `geometry_face_loops_counted` 가 tag 로 dispatch.
- **shared-vertex weld**: 인접 삼각형의 공유 정점은 LOCKED #5 spatial-hash dedup
  으로 하나로 붙어 연결 메시 형성. render 의 coplanar-edge hide (`EDGE_VISIBILITY_
  ANGLE`) 가 삼각형 대각선을 숨겨 tessellated quad 이 quad 로 보임.
- **정직한 skip**: 범위 밖 index / zero-area 삼각형은 세어서(dropped) 경고, 조작된
  face 안 만듦. classifier `SUPPORTED_GEOMETRY` += `IFCTRIANGULATEDFACESET`.

### 실측

| 입력 | 결과 |
|---|---|
| 삼각형 cube (12 tri, 8 pt) | **12 face / 8 vert** (공유 정점 weld), **valid watertight closedSolid**, cracks 0 |
| bad index / degenerate 삼각형 | dropped, 나머지만 임포트 |
| SketchUp `Building-Architecture.ifc` (12 tessellation set) | 이전 "no convertible geometry" → **10 부재 / 1067 face**, live 앱에 **집 모양** (지붕·벽·창) 렌더, 콘솔 오류 0 |

SketchUp 모델은 open shell / non-manifold 라 전체 `verify_invariants` 는 false —
이는 tessellated 소스 데이터의 본질 (viewing 용 reference geometry) 이지 임포트
버그 아님. 깨끗한 closed 삼각형 메시 (cube) 는 valid.

### 외부 커널 교차검증 (web-ifc)

`Building-Architecture.ifc` 를 web-ifc 로 메시화: **12 mesh / 1130 triangle**. 우리
임포터는 같은 12 부재를 1067 face 로 (삼각형 부재는 1 face/tri, extruded/brep 부재는
polygonal face — web-ifc 는 전부 삼각형화) — 두 커널이 같은 형상을 읽음.

### 회귀

- axia-ifc **+2**: `a_triangulated_face_set_becomes_one_face_per_triangle` (cube 12
  face, 각 face triangle) + `a_triangulated_face_set_skips_bad_triangles`.
- axia-wasm **+1** (WASM 경로, CI 가시): `a_triangulated_face_set_imports_as_a_
  welded_mesh` (12 face / 8 vert weld / valid). mutation 확인: `triangulated_face_
  set_loops` 를 empty 로 하면 0 face → 실패.
- 워크스페이스 **3176 passed / 0 failed / 1 ignored**(doc fence), vitest **2940**,
  tsc·build·ADR 카탈로그·외부 IFC(web-ifc) 통과. 양 WASM 타깃 byte-parity (4079233).

### 사용자 가치 — SketchUp / Revit 메시 임포트

이전엔 tessellated 모델 (SketchUp export 전체) 이 통째로 거부됐다. 이제 삼각형 메시
가 들어와 실제 형상 (집·가구·복잡 곡면) 을 본다. B-rep·swept·CSG (§25-29) 에 더해
tessellation 까지 — IFC 의 주요 geometry 표현이 모두 임포트된다.

### 남은 한계

- **삼각형 = DCEL face** 라 대형 모델 (수만 삼각형) 은 face 수가 많다 — 성능은
