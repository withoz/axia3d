// Tier 2 — boolean_union: A ∪ B (mesh-level CSG).
import { z } from 'zod';
...
export const booleanUnionCapability: CapabilityHandler<Input, Output> = {
  name: 'boolean_union',
  tier: 2,
  description:
    'Compute A ∪ B (fuse solids A and B into one). Both operand sets ' +
  ...
  handler: ({ engine }, input) => {
    ...
    const raw = engine.boolean_op(facesA, facesB, 'union');
