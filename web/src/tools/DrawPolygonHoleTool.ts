...
export class DrawPolygonHoleTool implements ITool {
  ...
  private finalize(): void {
    ...
    //     punch below would not: the loop fits its host face perfectly — it is
    //     the FAR side that is missing — so it succeeds, opens the closed solid,
    //     and reports "면 구멍을 뚫었습니다". Measured 2026-08-11 on a Ø80-bored
    //     200³ box: drill −1 (crossing), punch 41, closed true → false. Ask the
    //     engine with the same loop the drill got, and take its refusal seriously.
    if (this.ctx.bridge.drillProfileWouldCross(loop, normal)) {
      debugLog('[PolygonHole] Refused — the hole crosses an existing hole');
      Toast.warning(t('기존 구멍과 교차해 구멍을 뚫을 수 없습니다 — 위치를 옮겨 주세요'));
      return;
    }
