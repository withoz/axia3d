...
describe('ToolManager', () => {
  ...
  describe('sketch mode', () => {
    ...
    // ── The sketch basis must be the view-mode default's ──
    //
    // `getDrawPlane` derived screen-right as `up × normal` for a sketch while
    // the view-mode defaults carry theirs literally, and those three are not
    // generated by one cross product — the XZ wall's basis is left-handed with
    // respect to its +Y normal, the other two right-handed. So the derived
    // value disagreed with the default it was meant to reproduce:
    //
    //   XZ 정면  default +Z up / +X right   sketch  -Z up / +X right  upside-down
    //   YZ 측면  default +Z up / +Y right   sketch  +Y up / -Z right  both axes
    //   XY 바닥  default +Y up / +X right   sketch  +Y up / +X right  agreed
    //
    // The XZ `up` was -Z because that was the one value making the derived
    // right come out +X — the plane had been mislabelled a floor, where
    // "북쪽이 위" made -Z sound reasonable, and it outlived the relabelling.
    // Flipping it alone would have fixed the vertical by breaking the
    // horizontal, which is why the basis is carried now rather than derived.
    //
    // `right` is not decoration: DrawCircleTool, DrawArcTool, DrawBezierTool
    // and DrawEllipseTool all build their local frame from it.
    const CARDINAL = [
      { name: 'XZ 정면 (front wall)', action: 'sketch-start-xz', normal: [0, 1, 0], up: [0, 0, 1], right: [1, 0, 0] },
      { name: 'XY 바닥 (ground)',     action: 'sketch-start-xy', normal: [0, 0, 1], up: [0, 1, 0], right: [1, 0, 0] },
      { name: 'YZ 측면 (side wall)',  action: 'sketch-start-yz', normal: [1, 0, 0], up: [0, 0, 1], right: [0, 1, 0] },
    ];
    const axes = (v: THREE.Vector3) => [v.x, v.y, v.z].map((n) => Math.round(n * 1e6) / 1e6);

    for (const p of CARDINAL) {
      it(`${p.name} presents the basis it promises`, () => {
        tm.executeAction(p.action);
        // getDrawPlane is what the draw tools actually consult.
        const plane = (tm as any).getDrawPlane({ clientX: 400, clientY: 300 });
        expect(axes(plane.normal), 'normal').toEqual(p.normal);
        expect(axes(plane.up), 'screen-up is not what this plane promises').toEqual(p.up);
        expect(axes(plane.right), 'screen-right is not what this plane promises').toEqual(p.right);
      });
    }

    it('the two elevations put world-up on screen-up', () => {
      // The property that made -Z wrong, stated directly rather than as
      // literals: on a wall you are looking at, up is up.
      for (const action of ['sketch-start-xz', 'sketch-start-yz']) {
        tm.executeAction(action);
        expect(tm.getSketchInfo()!.up.z, `${action} draws upside-down`).toBeGreaterThan(0.999);
        tm.executeAction('sketch-exit');
      }
    });

    it('the basis stays orthonormal, so right cannot go stale', () => {
      for (const p of CARDINAL) {
        tm.executeAction(p.action);
        const plane = (tm as any).getDrawPlane({ clientX: 400, clientY: 300 });
        expect(plane.up.dot(plane.normal), `${p.name}: up off the plane`).toBeCloseTo(0, 6);
        expect(plane.right.dot(plane.normal), `${p.name}: right off the plane`).toBeCloseTo(0, 6);
        expect(plane.right.dot(plane.up), `${p.name}: axes not perpendicular`).toBeCloseTo(0, 6);
        expect(plane.right.length(), `${p.name}: right not unit`).toBeCloseTo(1, 6);
        tm.executeAction('sketch-exit');
      }
    });
