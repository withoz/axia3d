...
export class HistoryPanel {
  ...
  constructor(container: HTMLElement, callbacks: HistoryPanelCallbacks) {
    ...
    this.panelEl.innerHTML = `
      <div class="hp-header">
        <span class="hp-title">🕒 ${t('작업 기록 (Parametric)')}</span>
        <div class="hp-actions">
          <button class="hp-btn hp-btn-clear" title="${t('기록 전체 삭제')}">✕ ALL</button>
        </div>
      </div>
      <div class="hp-hint">
    ...
    this.panelEl.querySelector('.hp-btn-clear')?.addEventListener('click', () => {
      if (confirm(t('작업 기록을 모두 삭제하시겠습니까?'))) {
  ...
  private buildRow(e: OperationEntry): HTMLElement {
    ...
    row.innerHTML = `
      ...
      <button class="hp-rerun" title="${t('같은 연산을 새 값으로 재실행')}">${t('재실행…')}</button>
    `;
  ...
  private handleRerun(e: OperationEntry): void {
    ...
    if (dependents.length > 0) {
      ...
      const more = dependents.length > 3 ? t(' 외 {count}개', { count: dependents.length - 3 }) : '';
      const ok = window.confirm(
        t('⚠️ "{name}" 재실행 시 {count}개 후속 작업이 영향받습니다:\n', { name: e.displayName, count: dependents.length }) +
        `  ${names}${more}\n\n` +
        t('Phase 2 MVP는 자동 cascade 재계산을 아직 수행하지 않습니다 — 후속 작업은\n별도로 다시 실행해야 합니다. 계속하시겠습니까?')
      );
      if (!ok) return;
    }
    ...
    if (ok) {
      Toast.info(t('재실행 완료: {kind}', { kind: this.displayKind(e.kind) }), 2000);
    }
  }

  private promptLabelFor(kind: OperationKind): string {
    switch (kind) {
      case 'fillet-edge':    return t('필렛 반경 (mm) — 현재 선택된 엣지에 적용:');
      case 'chamfer-edge':   return t('챔퍼 거리 (mm) — 현재 선택된 엣지에 적용:');
      case 'thicken-faces':  return t('두께 (mm) — 현재 선택된 면에 적용:');
      case 'array-linear':   return t('선형 배열 "N, dx, dy, dz":');
      case 'array-radial':   return t('원형 배열 "N, axis(x|y|z), 총각도°":');
      case 'subdivide':      return t('(subdivide는 파라미터 없음 — Enter로 재실행)');
      case 'bend-selection':
      case 'twist-selection':
      case 'taper-selection':
        return t('{kind} — 각도/값:', { kind: this.displayKind(kind) });
    }
  }
