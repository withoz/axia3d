/**
 * ADR-097 T-δ — Topology Recovery Dialog (UI helper).
 *
 * Shown when `attemptAutoRecovery` returns `PartialFailure` (or when
 * the orchestrator escalates manual review). Modal with three options:
 *   - [Undo]      : revert via `bridge.undo()`
 *   - [강등]      : demote via `bridge.demoteXiaToShape` (caller wires)
 *   - [수동수정] : dismiss + show warning Toast with the reason
 *
 * The dialog is intentionally thin — orchestration logic (which
 * remediation to attempt, in what order, what reason text to surface)
 * lives in `TopologyRecoveryOrchestrator`. This module only renders
 * the modal and resolves a `Promise<TopologyRecoveryChoice>` based on
 * the user click.
 *
 * Design constraints:
 * - Pure DOM, no framework — can be tested in jsdom (Vitest default).
 * - Reason text is plain string; orchestrator humanizes engine
 *   damage labels before passing here.
 * - Backdrop click + ESC dismiss as 수동수정 (least destructive).
 *
 * v3.2 §12 Q5 사건 매핑:
 *   ADR-097 §B-T-G — escalation 시 사용자 선택지를 명시적으로 노출.
 */

export type TopologyRecoveryChoice = 'undo' | 'demote' | 'manual';

export interface TopologyRecoveryDialogOptions {
  /** Human-friendly reason (Korean), e.g. "면 3개 손상 (degenerate)". */
  reason: string;
...
const DIALOG_ID = 'axia-topology-recovery-dialog';
 ...
 * to avoid stacking modals (defensive — orchestrator should serialize).
 */
export function showTopologyRecoveryDialog(
  options: TopologyRecoveryDialogOptions,
): Promise<TopologyRecoveryChoice> {
  const doc = options.doc ?? (typeof document !== 'undefined' ? document : null);
  if (!doc) return Promise.resolve('manual');

  // Defensive: only one modal at a time.
  ...
  return new Promise((resolve) => {
    let resolved = false;
    const finish = (choice: TopologyRecoveryChoice) => {
      if (resolved) return;
      resolved = true;
      cleanup();
      resolve(choice);
    ...
    title.textContent = '위상 손상 자동 복구 실패';
    ...
    const mkBtn = (label: string, choice: TopologyRecoveryChoice, primary = false) => {
