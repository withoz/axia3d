...
| 수리가 **정점을 두 번 지나는 면**을 만듦 | `polygon_difference_by_clip` | 떨어진 재방문을 거르지 않았다 |
...
그런 차집합은 **한 점에서 꼬집힌 두 영역**이지 한 폴리곤이 아니다. 이제 그렇게
말하고 거절한다. 겹침은 남지만 — 수리가 닿기 전 상태 그대로 — 면은 다시 읽힌다:

```
was   FaceId(37)  31정점, 중복 2   → 교차점 0, lens 0      (읽을 수 없음)
is    FaceId(4)   23정점, 단순     → 교차점 4, lens 8정점   (진짜 두 번 베어 문 겹침)
```

**L-105-9** 차집합 결과가 **떨어진 위치에서** 자기를 만나면 거절한다
(`polygon_difference_by_clip`). 연속 중복 제거만으로는 부족하다 — 재구축의
`add_vertex` dedup 이 그걸 하나의 VertId 로 접는다.
