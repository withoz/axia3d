...
## ⚑ 작업 방식 — 매 세션 시작 전 (2026-08-04)

아래는 **이 저장소에서 실제로 저지른 실수**에서 나왔다. 근거와 사례는
`docs/WORKING_PRINCIPLES_2026-08-04.html` 참조.

**작업 전**
- 메모리와 기존 기록을 먼저 읽는다. 이미 적어 둔 함정을 다시 밟지 않는다.
- 코드를 읽고 원인을 "추론"하지 않는다. 계측하고 측정한다.
- `git branch --show-current` 를 확인한다. 기계를 떠나는 작업(push/PR/merge/
  삭제) **직전에 다시** 확인하고, 대상을 먼저 말한다.

**전체 연결을 볼 곳 — 이 파일이 아니다**
- **배선 지도**: `web/src/commands/ActionWiring.test.ts` 헤더. DOM 문자열부터
  Rust 까지 6층과 각 링크를 붙잡는 검사. `CONTAINERS` 가 그 지도를 **데이터로**
  갖고 있어 검사가 거기서 생성된다 — 새 DOM 컨테이너가 생기면 테스트가 깨진다
  (2026-08-10 에 툴바 75개 진입점이 모든 가드 밖에 있었던 이유).
- 조용히 깨지는 링크는 **둘뿐**이다: DOM `data-action` 문자열, 그리고 bridge 의
  `this.engine.X?.()` (optional 이라 export 가 사라져도 tsc 가 못 본다). 나머지는
  컴파일러가 잡는다 — 그래서 가드가 그 두 곳에 모여 있다.
- **이력은 ADR + LOCKED + `git log`** 가 canonical. 메모리는 "지금 무엇이 참인가"
  만 담는다 (그래서 2026-08-10 에 2308줄 → 733줄로 줄였다).

**판단할 때**
- 측정하지 않은 것은 말하지 않는다. 추정치를 근거로 우선순위를 정하지 않는다.
- 아무것도 안 나온 측정은 "차이 없음"이 아니라 **"묻지 않았음"** 일 수 있다.
  질문이 도달했는지부터 확인한다.
- 테스트를 대상 코드와 같은 믿음으로 쓰지 않는다. 결과물을 읽는다.

**바꿀 때**
- 커밋하고 나서 변형한다. `git checkout --` 로 되돌리지 않는다 (`cp` 백업으로).
- 한 번에 한 가지만. 고치는 김에 다른 걸 함께 바꾸지 않는다.
- 가드를 넣었으면 그 가드를 깨서 실패를 눈으로 본다. **통과는 증거가 아니다.**
- 잡히지 않는 변이는 억지 픽스처로 메우지 말고, 왜 관측 불가인지 적는다.

**끝내기 전**
- 변경이 닿는 모든 검사를 돌린다. **도구의 동작이 바뀌었으면 E2E 도** 돌린다.
- Playwright 전에는 반드시 `npm run build`. `dist/` 는 자동으로 따라오지 않는다.
- 결과를 있는 그대로 보고한다. 실패했으면 실패라고, 건너뛰었으면 건너뛰었다고.
...
MCP 배선 가능 capability 9개 ✅ (`640e8a3`, get_schema_version/get_xia_geometry_state/
...
### 96. ADR-291 — Plane-Cut Integrity Gate Consistency (trim / curved knives, 2026-07-12) ✅

**Canonical anchor (사용자 결재, 2026-07-12)**: 정찰이 "trim 게이트 누락 = confirmed
silent corruption" 로 플래그 → 사용자 결재 C → measure-first 로 전제가 두 번
뒤집힘 → 사용자 결재 "verify first" → "(b) 최소 정합성 (integrity_gate mirror, SI 아님)".

**측정 우선 (메타-원칙 #6) — 전제 전복**:
1. **trim / cut_curved / trim_curved 는 견고** — degenerate/grazing 평면(tangent/
   vertex/thin-sliver/tilted-corner/compound) 전부 SELF-REJECT 또는 SAFE. trim
   corruption 재현 0.
2. **`slice`(양쪽 유지)만** thin(z=49.9→SI 17)/tilted-corner(SI 12)에서 Ok-commit.
3. **결정적**: 같은 평면에서 **trim(한쪽 유지)은 SI-clean** → slice SI는 **두
   반쪽이 절단면에서 접하는(coincident)** keep-both 본질, 결과 solid corruption 아님.
4. **`damage_count = invariant_violations + geometric_cracks + open_boundary_edges`
   — SI 제외** → slice의 `integrity_gate_passed`는 SI를 원천적으로 못 봄. SI-aware
   게이트는 정상 slice(두 반쪽이 절단면에서 접하는 모든 경우) false-reject → **금기**
   (과-게이팅, 메타-원칙 #6).

**결정 ((b) 최소 정합성)**: slice의 **`integrity_gate_passed`(crack/invariant/
open-boundary, baseline-relative, SI 아님)** 를 `trim_volume_by_plane` +
`cut_curved_by_z_plane` + `trim_curved_by_plane` 의 routed Ok arm 에 mirror
(manual_txn=false, Scene 내부 commit). slice와 동일 safety net(공유 slice 코어의
future crack 회귀 방어 — ADR-267 "모든 plane-cut 게이트" 불변) + clean 결과 no-op +
SI false-reject 0 + regression guard 의 trim 누락 해소. merge 는 out-of-scope
(corruption 측정 0, closure-gate SI-aware 계열).

#### Lock-ins (L-96-1 ~ L-96-8)
- **L-96-1** 3 plane-cut op 이 `integrity_gate_passed(..., "trim"|"cut curved"|
  "trim curved", false)` 호출 (routed Ok arm, slice mirror).
- **L-96-2** 게이트 범위 = crack/invariant/open-boundary만 (OpenMesh damage_count,
  baseline-relative). **SI 아님.**
- **L-96-3** plane-cut op 에 SI 게이트 금지 (inter-half touching 은 corruption
  아님, 정상 slice false-reject). rationale 는 `phase3_gate_sim`
  `adr291_derisk_classify_slice_self_intersections` 하드 assert(trim keep-one
  SI-clean)로 lock.
- **L-96-4** manual_txn=false (Scene 내부 commit → 실패 시 discard_last_undo;
  not-routed/error arm 은 frame 없음).
- **L-96-5** baseline-relative — 곡면 Path B rim artifact 가 false-reject 안 함.
- **L-96-6** source-grep regression guard (`step6` `adr267_gamma_…`) 확장: ≥13
  gate 호출 + `"trim"|"cut curved"|"trim curved"` label 명시.
- **L-96-7** merge 는 out-of-scope (측정 corruption 0).
- **L-96-8** 절대 #[ignore] 금지.

#### 회귀
axia-geo +2 (`phase3_gate_sim` adr291 de-risk — trim keep-one SI-clean 하드
assert) / step6 guard +3 label (count 10→13) / Playwright +2
(`adr-291-cut-trim-integrity-gate` — clean box trim 6→6 valid + Path B cylinder
curved slice routed valid). workspace cargo green (1 pre-existing slow-channel
ignored). tsc/build 정상.

#### 라이브 검증 (real Chromium E2E)
clean box `trimVolumeByPlane`(z=0 keepAbove) → ok, faces 6→6, valid, integrity
clean (게이트 transparent no-op). Path B cylinder `cutCurvedByZPlane`(mid-z slice)
→ ok routed, valid. → 게이트가 wired + clean cut 에 투명(회귀 0) 임을 full WASM
stack 으로 증명.

#### Cross-link
- ADR-291 본문 §D (`docs/adr/291-plane-cut-integrity-gate-consistency.md`)
- ADR-267 (integrity_gate_passed — mirror source) / ADR-272/273 (closure gate +
  SI checker — SI-aware 계열, 의도적 미사용) / ADR-241 (polygonal trim) /
...
#### K/Tab 키 핸들러 wiring (follow-up LANDED 2026-07-13)
audit 결과 핸들러는 **이미 존재**(KeyboardShortcuts.ts:217-243), findSnap 재활성으로
live 복귀. 3 fix: (1) K `stopImmediatePropagation`(K 가 Back view 로도 점프하던
same-window listener 충돌 해소) / (2) `SnapManager.getActiveTentative` +
applyObjectSnap 가 findSnap 前 우선(guard `!hasLockedInference`) → Tab-cycle 픽이
commit 에서 유지(mousedown 이 findSnap 재실행 → index 리셋 문제 해소) / (3) mousemove
listener `resetTentative` + cancelCurrentTool/notifyViewModeChange/setTool 에
`clearLockedInference`+`resetTentative`(lock/tentative 가 intent 경계 넘어 leak —
ADR-166 plane-lock 과 의도적 divergence: plane-lock 은 setTool 유지, inference-lock 은
transient). 회귀 vitest +5(lock reset Esc/view/tool + tentative no-op) / Playwright +1
(K-lock 이 applyObjectSnap 통해 commit 구속 + tool switch 시 해제). 1023 tool/snap/
keyboard tests green, tsc 0. mid-draw Tab(2nd+ click)은 VCB 필드 cycle 이 capture 로
소유 → idle-only(별도 큰 변경).

#### Deferred (follow-up)
mid-draw Tab tentative(tool.onKeyDown 경유) / OffsetTool getGroundPoint 경로 snap /
localStorage 영속 / guide dashed line polish.
...
### 103. 입체면 위 곡선 도형 — 겹침 분할 · 타원 · 곡선 샘플링 (2026-08-05) ✅

**재-오진 방지 (LOCKED #88 의 목적)** — 아래 셋은 **고쳤다. 다시 결함으로 올리지 말 것.**
근거는 각 테스트 파일 머리주석에 실측과 함께 있다.

| 실측 전 | 지금 |
|---|---|
| 입체면 위 겹치는 원 2개 → 두 번째 **거부** | lens + 초승달 2개, rim 은 arc 유지 |
| 입체면 위 **타원 아예 못 그림** (작은 것도) | 그려진다 (9 ms) |
| 타원 1개 tessellate = **4096점** (=상한) | **128점**, 허용오차가 실제로 작동 |

- **겹치는 두 원** (`operations/circle_overlap.rs`): 두 원이 만나는 두 점이 각 rim 을 둘로
  가른다 — lens = 안쪽 호 2개, 초승달 = 바깥 호 + 상대 안쪽 호 뒤집기, 그리고 **담고 있던
  면의 구멍은 두 개가 아니라 바깥 호 2개짜리 합집합 하나**가 된다. 각 호는 중점에서 한 번
  쪼갠다 (2-edge bigon 회피 — re-derive materializer 와 같은 이유). 시트에서는 arrange 가
  이미 하던 일이고, **입체에서는 그 re-derive 가 의도적으로 건너뛰어진다**(벽이 딛고 선
  edge 를 다시 자르므로) — 그래서 별도 경로. 회귀 `two_overlapping_circles_become_three` /
  `two_circles_that_cross_on_a_solid`.
- **타원 containment**: `assign_circle_holes_innermost` 가 원만 받던 것을 **모든 closed
  curve** 로. 시트에서 이걸 하던 freeform containment(ADR-186 A2)가 re-derive 안에 있어서
  입체에선 아무도 안 했다.
- **곡선 샘플링** (`curves/{bspline,nurbs}::tessellate`): 점 개수를 `제어폴리곤 길이 ÷
  허용오차` 로 잡고 있었다 — **호 길이당 한 점**이라 곡률과 무관. bezier 는 원래 재귀
  세분을 했고, 이제 셋 다 `curves::refine_chord` 하나를 쓴다. **매듭은 항상 샘플**(꺾일 수
  있는 자리). 효과: 입체 위 타원 그리기 36.2초 → 39.8ms, 겹침 스캔 12.15초 → 12.4ms,
  export 248ms → 1.04ms, **axia-geo 라이브러리 스위트 50.6초 → 1.7초**.
- **거부 메시지**: "면이 겹치지 않게 그려주세요" 는 이제 대체로 **틀린 조언**이다. 겹치는
  경계의 종류에서 유도한다 — 원↔각진 도형이면 *"원을 먼저 그리면 나뉜다"* (실측된 유일한
  우회), 타원·자유곡선이면 **순서를 제안하지 않는다** (어느 순서도 안 되므로).

~~**아직 남은 것 (실측, 입체면 한정 — 시트는 전부 됨)**: rect 를 먼저 그린 뒤 원을 겹쳐 그리기
(반대 순서는 됨) / 타원이 무엇과든 겹치는 경우.~~

> ⚠ **둘 다 닫혔다 (2026-08-30 실측 확인).** 이 문단은 첫 수정 **하루 전**(08-05)에 쓰였다.
> `a_refusal_says_which_overlap_it_was` 가 스스로 기록하듯 — **08-06** 에 입체용 coplanar
> arrangement 를 *되켜서* `rect → circle` 이 `circle → rect` 와 똑같이 아홉 면으로 나뉘고,
> **08-09** 에 `is_reverse_twin` 의 freeform arm 이 추가되며 타원 행도 닫혔다. 지금 그 파일의
> 단언은 `a_circle_and_a_rectangle_divide_each_other_either_way` 와
> `an_ellipse_overlap_divides_cleanly` 이고 둘 다 통과한다. **순서를 조언할 것이 없다.**

> ⚠ **위 문단은 2026-08-13 재실측으로 정정** — 두 잔존 모두 **이미 나뉜다.** 이 실측
> (08-05)은 입체 top 재타일(ADR-281 β-1) 활성화 **하루 전**이었고, 약속된 은퇴-신호
> 테스트 `a_refusal_says_which_overlap_it_was` 는 **저장소에 존재한 적이 없다** (grep 0건)
> — 그래서 고쳐졌는데 아무것도 안 울렸고, 확장계획(PR #123)까지 D8 로 실어 날랐다.
> 재실측 (프로덕션 4 플래그, 분할 판정 = 면 개수가 아니라 **top 40,000 정확 분할**):
> rect→circle 40,000.0000 / circle→rect 40,000.0000 / circle→circle 40,000.0000 —
> 전부 위반 0. 이제 진짜 잔존은 `what_divides_on_a_solid_top.rs` 가 **실존하는 핀**으로
> 고정한다: ① **코너 걸침의 L자 바깥 조각 소실** (엣지 걸침의 볼록 바깥은 생긴다 —
> 45,000 정확; 코너의 비볼록 바깥만 사라진다, 계획서 D1 의 국소화) ② **rect×ellipse 의
> union 구멍 루프가 곡선 메타데이터를 잃어** 구멍이 현 다각형으로 ~5% 작게 공제
> (40,237.88 — 조각들은 곡선을 유지하는데 구멍만 잃는다). 나뉘는 것 자체는 타원 포함
> 전부 나뉜다 (freeform 플래그 = 프로덕션 ON 기준).
...
> ⚠ **둘 다 지났다 (2026-08-30 실측 확인). 이 두 문장을 근거로 작업을 고르면 헛돈다 —
> 실제로 그랬다.**
>
> **곡면 위 선** — 상태기계 변경은 **2026-08-25 에 이미 되었다**. `DrawLineTool` 이 곡면
> host 에서 체인을 모으고 3점 이상을 요구하며 `drawOpenSeamOnCurved` 를 한 번 부른다
> (`c3-line-seam-on-a-curved-face.spec.ts`). ⚠ 다시 구현하지 말 것.
>
> 그리고 그것은 **원뿔에서만** 되고 그것이 정상이다 — **닫힌 곡면은 열린 호로 나뉘지
> 않는다.** 원뿔 옆면은 밑판과 밑 rim 을 공유하니 rim 이 경계지만, 앱의 구는
> `create_sphere_axis_native` 로 **면 하나 + 내부 자오선 seam + 경계 없음**이다. 실측
...
> | 원통 · 토러스 | **나눔** | 안 됨 |
>
> **계획 C** — 그 전제("입체 위에서 coplanar arrangement 가 통째로 꺼져 있다")는
> **2026-08-06 에 되켜지며 사라졌다**. LOCKED #103 의 amendment 참조.
...
12 세션 × 20 연산 결정적 퍼즈에서 **12 세션 전부 완주**한다 (이전 9).
`KNOWN_BREAKS` 가 비었다.

| 무엇이 | 어디서 | 무엇이 없었나 |
|---|---|---|
| 두 벽을 가로지른 사각형이 **가운데 조각을 두 번** | `split_polygon_2d` | 교차점을 **첫 선분 방향으로만** 정렬해 짝지음 |
| 평면에 **선 채로 서 있는 벽**을 관통으로 세어 같은 영역 3번 | `split_faces_crossing_other_planes` | **롤백이 없었다** (형제 둘에는 있었다) |
| 밀어넣기 뚜껑이 **원의 활꼴 시트를 덮음** | `Command::CreateSolid` 디스패치 | 이중덮임 수리가 **그리기 뒤에만** 돌았다 |
| 재유도가 z=0 에 **같은 자리 두 면** | `rebuild_coplanar_faces_analytic_scoped` | 롤백이 **자기교차만** 물었다 |
| 오목해진 면 위 겹침을 **재지도 못함** | `polygon_difference_walking` | 안쪽 경계를 **lens** 에서 가져왔다 |
| 수리가 **정점을 두 번 지나는 면**을 만듦 | `polygon_difference_by_clip` | 꼬집힌 결과를 **한 조각으로** 돌려줬다 |

#### 세 줄 요약 — 넷 다 같은 모양

**일치(coincident)는 교차(crossing)가 아니다.** `detect_self_intersections` 는 같은
자리를 덮는 두 면을 세지 못한다 — 서로를 *지나가는* 게 아니라 *위에 눕는* 것이라서.
그걸 묻는 가드는 조건이 다 맞아도 눈이 멀어 있었다:

```
z=0 위 솔리드 면 — 평면 3, 곡면 0     가드 조건 성립
자기교차   19 → 19                    가드가 묻는 것: 그대로
불변식위반  0 → 1                     실제로 일어난 일
```

그래서 셋은 **불변식 검사를 (상대적으로) 함께 묻도록** 했고, 하나
(`split_polygon_2d`)는 짝짓기 자체를 고쳤다.

#### 각 수정의 lock-in

- **L-105-1** `split_polygon_2d` 는 세그먼트를 **연결된 경로**로 묶어 **한 경로씩**
  자른다. 경로가 하나면 옛 코드로 그대로 간다 (원래 되던 경우는 새 코드를 안 지남).
  ⚠ **collinear 기준은 틀렸다** — 모서리를 도는 절단이 여섯 선분으로 와서 `12 → 12`
  로 안 잘렸다. 끝점을 공유하는 것들이 함께 필요했던 것들이다.
- **L-105-2** `split_faces_crossing_other_planes` 는 없던 위반을 남기면 스스로
  되돌린다. 접촉과 관통을 미리 가리는 건 별개 문제이고, 결과는 의심의 여지가 없다.
- **L-105-3** `Command::CreateSolid` 는 이중덮임 수리를 돌리되 **밀어넣기가 실제로
  깨뜨렸을 때만**. ⚠ 매번 돌리면 면 ID 가 갈리고 **다음 연산이 면을 못 찾는다**
  (실측: `"face FaceId(16) not found or inactive"`) — 막으려던 겹침보다 나쁘다.
  그래서 가드는 면 개수가 아니라 **명령 결과**를 읽는다.
- **L-105-4** 재유도 롤백은 자기교차에 더해 **불변식 위반**도 (상대적으로) 본다.
- **L-105-5** 전부 변이검증 통과 (가드를 깨면 실패하는 것을 눈으로 봄) — 단
  아래 §무엇이 값을 했는지 의 마지막 줄은 예외이고, 그렇게 적었다.
...
```
이전   10 → 20면, 겹친 쌍 있음
이후   10 → 11면, 건전, 그러나 분할 안 함
```

**그리기는 거절되지 않는다** ("A DRAW IS NEVER REFUSED", 2026-08-06) — 거절되는 건
*분할*이다. 재유도가 자기교차에 대해 이미 하던 것과 같은 거래이고, 새로운 건 그
스캔이 못 보던 손상까지 본다는 점뿐이다. 스펙(`web/e2e/what-a-draw-does-beside-a-
solid.spec.ts`)이 **더해진 면이 하나임을 단언**해 두어, 나중에 제대로 분할하게 되면
그 시험이 실패하며 무엇이 고쳤는지 말하게 된다.

#### 마지막 하나 — 차집합이 lens 를 걷고 있었다

세션 3 은 op 10 → 13 → 15 로 두 번 밀린 뒤 완주했다. op 15 의 원인은 **차집합
walk 이 안쪽 경계를 lens 에서 가져온 것**이다. 앞선 연산들이 나눠 놓은 면은 오목
하고, 오목한 대상에 대해 Sutherland-Hodgman 의 lens 는 **한 폴리곤이 아니다** —
떨어진 두 조각을 0폭 연결선으로 이어 돌려준다 (실측: 20×20 두 정사각형이 8정점
하나로, 넓이 800 은 정확).

`polygon_difference_by_clip` 은 **클립 폴리곤 자신**을 걷는다. 한 입의 경계는
`대상의 몫 + 클립의 몫` 이고, 클립엔 그런 문제가 없다. 교차점이 짝수면 몇 개든
같은 방식으로 걷는다.

```
퍼즈 12 세션 × 20 연산 → 12/12 완주.  KNOWN_BREAKS 가 처음으로 비었다.
```

**L-105-6** 안쪽 경계는 lens 가 아니라 **클립**에서 온다 (`polygon_difference_by_clip`).
**L-105-7** SH 는 **클립**만 볼록하면 된다 — 대상 오목 허용 (가드가 둘로 갈림:
비볼록 클립은 여전히 거절, 비볼록 대상은 이제 잰다).
**L-105-8** 후진 splice 의 정지 조건은 push **앞**에 (뒤면 출구 변 시작점까지 밀려
호가 되돌아간다 — ⚠ **넓이로는 안 보인다**, 0-넓이 excursion 이라 8000 그대로).

#### ⚠ 무엇이 값을 했는지 — 변이로 하나씩

| 변경 | 되돌리면 | 잡는 것 |
|---|---|---|
| lens → **클립** 걷기 | op 15 재발 | **퍼즈** |
| splice 정지 조건 | 호가 되돌아감 | 2D 시험 2건 (정점 단언) |
| 대상 볼록 완화 | 오목 대상 거절 | `adr101_phase_b2_non_convex_face_errors` |
| 짝수 교차 게이트 | `!= 2` | **없음** |

마지막 줄은 정직하게 적는다: `!= 2` 로 좁혀도 퍼즈는 초록이다. 4-교차 겹침을 만들
면서 **수리도 필요한** 장면이 아직 없다. 능력 자체는 단위 시험이 붙잡고 있고
(`a_concave_subject_bitten_twice_by_a_convex_clip`), 이 게이트는 장면이 거기 닿게
하는 배선이다 — 이제 잴 수 있는 겹침을 거절하는 것이 더 나쁜 답이라 남겼다.

#### 마지막 하나 — 두 계측기가 서로 다른 말을 한 이유

6-연산 축소에 남은 쌍에서 두 계측기가 정반대를 말했다:

```
edge_stacked_face_pair          "같은 자리를 덮는다"
coplanar_intersection_segments  "교차점 0, lens 0"  (안 만난다)
```

**둘 다 옳았다.** 사이에 있던 건 **정점을 두 번 지나는 면**이었다 — 바깥 루프
31정점에 중복 2개, 원을 그린 뒤 꼬리가 나갔다 이미 쓴 점들로 돌아온다. 국소
검사는 공유 엣지에서의 방향만 보니 맞게 답했고, 클립은 자기접촉 고리를 SH 에
넣으니 아무 말이나 한 것이다.

**만든 것은 수리다** (수리를 끄면 op 5 의 잘못된 루프 0, 켜면 1). 차집합 walk 이
**이미 지난 점으로 돌아오는 고리**를 돌려줬고 — `dedup_ring_2d` 는 **연속** 중복만
지운다 — 재구축이 모서리마다 `add_vertex` 를 부르는데 그게 0.15μm 로 합치니
(LOCKED #5) 같은 VertId 를 두 번 지나는 루프가 됐다.

그런 차집합은 **한 점에서 꼬집힌 두 영역**이지 한 폴리곤이 아니다. 거절이 첫 답
이었고 그때는 겹침이 남았다. **조각들이 진짜 답이다** — P 를 두 번 지나는 고리는
첫 P 에서 둘째 P 까지의 고리 + 둘째 P 에서 돌아 첫 P 까지의 고리이고, 넓이가 정확히
더해진다 (실측: 8정점 고리 → [2500, 2500] 합 5000, 각 조각 단순).

```
was          FaceId(37) 31정점, 중복 2  → 교차점 0, lens 0      (읽을 수 없음)
거절만 했을 때  겹침 1건 남음             → 교차점 4, lens 8정점   (읽히지만 안 나뉨)
is           조각 둘로 세움              → 위반 0, 면 32 → 33     (나뉨)
```

#### ⚠ "수리를 없애면 되나" — 재 봤고, 아니다

두 호출부(그리기 뒤 · 밀어넣기 뒤)를 다 끄면:

```
3741 통과 / 8 실패     퍼즈 세션 10 이 op 11 에서 다시 깨짐
```

`blast_radius` · `a_second_rect_nested_in_the_first_still_takes_its_piece` ·
`the_same_draw_over_a_one_shot_face_now_resolves_too` 등이 수리에 기대고 있다.
**수리가 도는 것이 문제였던 적은 없다** — 둘인 것을 하나로 돌려준 것이 문제였다.

**L-105-9** 차집합은 **조각들**을 돌려준다 (`polygon_difference_by_clip` →
`Vec<Vec<_>>`). 떨어진 위치에서 자기를 만나는 고리는 그 점에서 정확히 갈라진다
(`split_ring_at_self_touches`). 연속 중복 제거만으로는 부족하다 — 재구축의
`add_vertex` dedup 이 그걸 하나의 VertId 로 접는다. 조각은 전부 원래 면의 주인을
물려받는다.
**L-105-10** 자기접촉 루프를 가진 면은 **어떤 계측기도 읽을 수 없다.** 두 계측기가
...
#### 이 세션이 스스로에게 당한 것 (같은 함정 재발 방지)

- **`#[test]` 가 지워진 채 초록** — 시험을 정답으로 뒤집을 때 속성이 사라져,
  옛 동작을 되돌리는 변이에서도 통과했다. 이 저장소가 반복해 당한 vacuous check
  (ADR-299/302/304, LOCKED #100 L-100-5) 의 또 한 얼굴.
- **`--no-fail-fast` 없이 낸 결론** — 첫 실패 바이너리에서 멈춘 실행을 근거로
  "고쳐지지 않았다"고 보고했다가 정정. **워크스페이스 판단은 `--no-fail-fast`.**
- **워크트리에서만 실패하는 가드** — `occtRuntime.test.ts` 가 후보 경로를 워크트리
  루트에서 멈춰, hoist 된 `node_modules` 에 닿지 못했다. 위로 걸어 올라가도록 고침
  (Node 가 import 를 푸는 방식과 같은 질문).
- **다른 크레이트 스위트를 돌려 놓고 "가드가 없다"고 보고** — 완화를 붙잡는 가드는
  `axia-geo` 에 있었는데 `axia-core` 를 돌렸다. 변이가 안 잡히면 **먼저 그 가드가
  있는 곳을 돌렸는지** 확인한다.
- **넓이만 단언하는 가드** — 0-넓이 excursion 은 넓이로 안 보인다. 폴리곤을 고칠
  땐 **정점을** 단언한다.

#### 회귀 자산

axia-geo / axia-core / axia-wasm **3744 통과 / 0 실패** · vitest **3101 통과 /
1 건너뜀** · Playwright **285 통과 / 1 건너뜀**. 새 가드는 전부 변이검증됨.

#### 알려진 환경 공백 (이 세션과 무관, 결함 아님)

`packages/axia-mcp-server` 의 `zod` · `@modelcontextprotocol/sdk` 가 메인 저장소
`node_modules` 에도 없어 MCP 스위트가 안 돈다. `npm install` 이 필요한 상태이고,
이 세션은 MCP 파일을 **하나도** 건드리지 않았다.

#### Cross-link

LOCKED #41 (ADR-101 — `split_polygon_2d` / 비볼록 §5) · #76 (ADR-176 자동 동작
production ON — 플래그는 그대로, 판정만 추가) · #88 (doc-lag 이 오판을 낳은 기록) ·
#100 (L-100-5 "검증됨" 주장은 단언하는 시험을 대야) · 메타-원칙 #4 (SSOT) ·
#6 (Preventive / measure-first) · #16 (자동화 antipattern)
