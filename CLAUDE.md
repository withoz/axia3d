...
12 세션 × 20 연산 결정적 퍼즈에서 **11 세션이 완주**한다 (이전 9). 남은 하나는
아래 §남은 것.
...
- **L-105-5** 넷 다 변이검증 통과 (가드를 깨면 실패하는 것을 눈으로 봄).
...
세션 3 만 남았고, op 10 → 13 → **15** 로 두 번 밀렸다. 여섯 연산으로 줄었고 밀어넣기
뚜껑이 **앞선 연산들이 비볼록으로 만든 원 면** 위에 내려앉는다:

```
coplanar_intersection_segments → Err(coplanar clipping requires convex faces;
                                     face FaceId(4) is non-convex)
```

`subtract_double_covered_faces` 의 세 갈래가 **전부** 통과한다 — 겹침을 *재지도*
못한다. **대상 볼록 요구를 푸는 것도 해 봤다** (Sutherland-Hodgman 은 **클립**만
볼록하면 된다): 재기는 된다 (교차점 4, lens 8) 그러나 차집합 walk 은 **정확히 2개**만
다뤄 아무 갈래도 작동하지 않는다. **아무것도 벌지 못하는 채로 두지 않고 되돌렸고**,
측정은 `session_3_when_the_rollbacks_are_in.rs` 머리주석에 남겼다. 일반 비볼록
클리핑은 ADR-101 §5 의 별도 항목이다.
